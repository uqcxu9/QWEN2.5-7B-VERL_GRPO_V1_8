...
def _profiles_json_path() -> str:
    """Resolve data/profiles.json without relying on process cwd (VERL/Ray often cwd=Search-R1)."""
    override = os.environ.get("ECON_PROFILES_JSON_PATH", "").strip()
    if override and os.path.isfile(override):
        return override
    # .../ai_economist/foundation/scenarios/one_step_economy/this.py -> repo root (parent of ai_economist)
    here = os.path.dirname(os.path.abspath(__file__))
    repo_data = os.path.normpath(os.path.join(here, "..", "..", "..", "..", "data", "profiles.json"))
    if os.path.isfile(repo_data):
        return repo_data
    legacy = os.path.abspath(os.path.join("data", "profiles.json"))
    if os.path.isfile(legacy):
        return legacy
    raise FileNotFoundError(
        f"profiles.json not found. Tried {repo_data!r}, {legacy!r}. "
        "Set env ECON_PROFILES_JSON_PATH to the absolute path."
    )
...
class OneStepEconomy(BaseEnvironment):
    ...
    def set_offer(self):
        with open(_profiles_json_path(), "r", encoding="utf-8") as file:
