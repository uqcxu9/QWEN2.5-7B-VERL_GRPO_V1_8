...
# Econ GRPO 冒烟（中等规模）：验证 GRPO 分组、advantage、pg_loss 与显存，不追最终收敛。
# DataLoader：EconStubDataset；每 epoch 步数 ≈ stub_dataset_size ÷ train_batch_size。
# 看日志重点：critic/rewards/min≠max（或 annual_reward 跨样本不同）、critic/advantages/* 不全为 0、
# actor/pg_loss 非零、无 OOM。若 OOM：先降 rollout_*_micro_batch、rollout_prompt_length、max_prompt_length。
...
export EXPERIMENT_NAME='econagent-verl-grpo-smoke'

# ---- 规模（单卡 7B 勿再大）----
# num_months 可改 1（更省显存）或 3（更多跨月差异）；replica_group_size=4 与 GRPO 分组一致（uid=group_key）。
SMOKE_AGENTS=100
SMOKE_MONTHS=3
REPLICA_GROUP_SIZE=4

# rollout/ref 按 micro 切块，避免 100×months×replica 条序列一次性顶满显存；OOM 时改为 4
ECON_ROLL_MICRO=8
...
PYTHONUNBUFFERED=1 "${TRAIN_PY}" -m verl.trainer.main_ppo \
  ...
  actor_rollout_ref.rollout.n_agent=$SMOKE_AGENTS \
  ...
  ++econ.enabled=true \
  ++econ.stub_dataset_size=2 \
  ++econ.project_root=/workspace/QWEN2.5_42_7b_main \
  ++econ.num_months=$SMOKE_MONTHS \
