    ...
    def validate(self, n_gpus: int, train_batch_size: int, model_config: dict = None):
        ...
        if not self.use_dynamic_bsz:
            if train_batch_size < self.ppo_mini_batch_size:
                # In econ mode, train_batch_size=1 (state bank entries) but actual
                # GRPO samples are 4800+. Warn instead of error.
                import warnings
                warnings.warn(
                    f"train_batch_size ({train_batch_size}) < "
                    f"actor.ppo_mini_batch_size ({self.ppo_mini_batch_size}). "
                    f"Allowed for econ mode where batch is inflated internally."
                )
