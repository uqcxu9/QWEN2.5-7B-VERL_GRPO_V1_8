...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        for _epoch_idx in range(self.config.ppo_epochs):
            if _epoch_idx > 0:
            ...
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                # DIAG: snapshot first parameter before optimizer step
                _diag_params = list(self.actor_module.parameters())
                _diag_p = _diag_params[0] if _diag_params else None
                _w_before = _diag_p.data.clone() if _diag_p is not None else None

                grad_norm = self._optimizer_step()
                mini_batch_metrics = {"actor/grad_norm": grad_norm.detach().item()}

                # DIAG: compare weights after optimizer step
                if _w_before is not None:
                    _w_after = _diag_params[0].data
                    _diff = (_w_after.float() - _w_before.float()).abs()
                    _bf16_changed = (_w_after.bfloat16() != _w_before.bfloat16()).sum().item()
                    _total = _w_after.numel()
                    print(f"[DIAG] param[0] shape={list(_w_after.shape)} dtype={_w_after.dtype}: "
                          f"fp32_diff_mean={_diff.mean().item():.2e}, "
                          f"fp32_diff_max={_diff.max().item():.2e}, "
                          f"bf16_changed={_bf16_changed}/{_total} ({100*_bf16_changed/_total:.2f}%)")
