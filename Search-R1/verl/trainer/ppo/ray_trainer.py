...
class RayPPOTrainer:
    ...
    def fit(self):
        ...
        # ---- EconAgent: initialize generation manager if econ enabled ----
        ...
        if econ_enabled:
            ...
            econ_gen_config = EconGenerationConfig(
                ...
                max_obs_length=self.config.data.get("max_obs_length", 256),
                num_gpus=self.config.trainer.n_gpus_per_node * self.config.trainer.nnodes,
                econ_project_root=str(OmegaConf.select(ec, "project_root", default="/workspace/QWEN2.5_42_7b_main")),
                state_bank_path=str(OmegaConf.select(ec, "state_bank_path", default="") or ""),
                ...
                rollout_agent_micro_batch=int(OmegaConf.select(ec, "rollout_agent_micro_batch", default=16)),
                rollout_prompt_length=int(OmegaConf.select(ec, "rollout_prompt_length", default=512)),
            )
            # Get vLLM server handles from the async rollout manager
            _server_handles = []
            for replica in self.async_rollout_manager.rollout_replicas:
                servers = getattr(replica, 'servers', None) or getattr(replica, 'server', [])
                if isinstance(servers, list):
                    _server_handles.extend(servers)
                else:
                    _server_handles.append(servers)
            _econ_generate_wrapper = EconVLLMGenerateWrapper(
                server_handles=_server_handles,
                tokenizer=self.tokenizer,
                rollout_config=self.config.actor_rollout_ref.rollout,
            )
            econ_generation_manager = EconGenerationManager(
                tokenizer=self.tokenizer,
                actor_rollout_wg=_econ_generate_wrapper,
                config=econ_gen_config,
                is_validation=False,
            )
            print(f"[econ] EconGenerationManager initialized: {econ_gen_config.num_agents} agents, "
                  f"{econ_gen_config.num_months} months, {econ_gen_config.replica_group_size} replicas")
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # ---- EconAgent: econ rollout replaces standard generation + reward ----
                if econ_enabled and econ_generation_manager is not None:
                    batch = DataProto(non_tensor_batch=batch_dict, meta_info={
                        "temperature": self.config.actor_rollout_ref.rollout.temperature,
                    })
                    with marked_timer("step", timing_raw):
                        # Sync FSDP weights to vLLM server before econ rollout
                        with marked_timer("update_weights", timing_raw, color="green"):
                            self.checkpoint_manager.update_weights(self.global_steps)

                        with marked_timer("gen", timing_raw, color="red"):
                            econ_generation_manager.timing_raw = timing_raw
                            batch = econ_generation_manager.run_econ_year_loop(
                                meta_batch=batch,
                                temperature=float(self.config.actor_rollout_ref.rollout.temperature),
                            )
                            # Cast tensor types
                            for key in batch.batch.keys():
                                if key in ('annual_reward',):
                                    batch.batch[key] = batch.batch[key].float()
                                elif batch.batch[key].is_floating_point():
                                    pass  # keep float tensors as-is
                                else:
                                    batch.batch[key] = batch.batch[key].long()

                        # Sleep vLLM replicas to free GPU memory for training
                        self.checkpoint_manager.sleep_replicas()

                        # Compute log probs for the econ rollout
                        with marked_timer("old_log_prob", timing_raw, color="blue"):
                            old_log_prob, _ = self._compute_old_log_prob(batch)
                            batch = batch.union(old_log_prob)

                        # Compute response_mask from attention_mask
                        ...
                        # Econ reward → token level scores → advantage
                        with marked_timer("adv", timing_raw, color="brown"):
                            ...
                            if _payload:
                                _pl = _payload if isinstance(_payload, dict) else (
                                    _payload[0] if isinstance(_payload, list) and _payload else {})
                                ...
                                if _reps:
                                    _env_keys = ("format_fail_count", "invalid_action_count",
                                                 "out_of_range_count", "negative_wealth_count",
                                                 "fallback_action_count")
                                    for _ek in _env_keys:
                                        _vals = [float(r.get("env_stats", {}).get(_ek, 0)) for r in _reps]
                                        metrics[f"econ_env/{_ek}"] = sum(_vals) / max(len(_vals), 1)
                        ...
                        # Set required meta fields for update_actor
                        batch.meta_info["global_token_num"] = torch.sum(
                            batch.batch["attention_mask"], dim=-1).tolist()

                        # Free cached GPU memory before heavy optimizer step
                        torch.cuda.empty_cache()

                        # Update actor
                        if self.config.trainer.get("critic_warmup", 0) <= self.global_steps:
                    ...
                    try:
                        ...
                        print(f"[econ] compute_data_metrics error: {e}")
                    try:
                        metrics.update(compute_timing_metrics(batch=batch, timing_raw=timing_raw))
                    except Exception as e:
                        print(f"[econ] compute_timing_metrics error: {e}")
                    print(f"[econ] sample metrics: score={metrics.get('critic/score/mean', 'N/A')}, "
                          f"R_macro={metrics.get('econ_reward/R_macro', 'N/A')}, "
                          f"pg_loss={metrics.get('actor/pg_loss', 'N/A')}, "
                          f"grad_norm={metrics.get('actor/grad_norm', 'N/A')}, "
                          f"kl_loss={metrics.get('actor/kl_loss', 'N/A')}, "
                          f"ppo_kl={metrics.get('actor/ppo_kl', 'N/A')}, "
                          f"clipfrac={metrics.get('actor/pg_clipfrac', 'N/A')}, "
                          f"entropy={metrics.get('actor/entropy', 'N/A')}")

                    # Checkpoint saving (same logic as standard path)
                    ...
                    # Validation (same logic as standard path)
                    ...
                    self.global_steps += 1
                    print(f"epoch {epoch}, step {self.global_steps - 1}")

                    if is_last_step:
                        progress_bar.close()
                        return
                    continue
                # ---- End EconAgent branch ----

                # Standard (non-econ) path: create batch from batch_dict
