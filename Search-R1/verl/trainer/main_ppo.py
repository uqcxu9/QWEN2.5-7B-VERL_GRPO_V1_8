...
class TaskRunner:
    ...
    def run(self, config):
        ...
        # ---- EconAgent: use state bank dataset when econ.enabled ----
        _econ_enabled = bool(OmegaConf.select(config, "econ.enabled", default=False))
        if _econ_enabled:
            import sys as _sys
            _econ_root = str(OmegaConf.select(config, "econ.project_root", default="/workspace/QWEN2.5_42_7b_main"))
            if _econ_root not in _sys.path:
                _sys.path.insert(0, _econ_root)

            train_dataset, val_dataset, collate_fn = _create_econ_datasets(config)
            train_sampler = None
        else:
            # Standard RLHF dataset path
...
def _create_econ_datasets(config):
    """Create EconAgent state-bank datasets and collate function."""
    import numpy as np
    from omegaconf import OmegaConf
    ...
    class EconStubDataset(Dataset):
        def __init__(self, n_items, default_year_index):
    ...
    _sbp = OmegaConf.select(config, "econ.state_bank_path", default=None)
    _sbp_s = str(_sbp).strip() if _sbp is not None else ""
    _def_y = int(OmegaConf.select(config, "econ.default_year_index", default=2))

    # Separate val state bank: look for state_bank_val.json next to state_bank_train.json
    _sbp_val = OmegaConf.select(config, "econ.state_bank_val_path", default=None)
    _sbp_val_s = str(_sbp_val).strip() if _sbp_val is not None else ""
    if not _sbp_val_s and _sbp_s:
        # Auto-detect: if train path ends with _train.json, try _val.json
        if _sbp_s.endswith("_train.json"):
            _candidate = _sbp_s.replace("_train.json", "_val.json")
            if os.path.isfile(_candidate):
                _sbp_val_s = _candidate

    if _sbp_s and os.path.isfile(_sbp_s):
        train_dataset = EconStateBankDataset(_sbp_s, _def_y)
        if _sbp_val_s and os.path.isfile(_sbp_val_s):
            val_dataset = EconStateBankDataset(_sbp_val_s, _def_y)
            print(f"[econ] val dataset: {_sbp_val_s}")
        else:
            val_dataset = EconStateBankDataset(_sbp_s, _def_y)
            print(f"[econ] WARNING: no separate val state bank found, using train set for val")
    else:
        _stub_n = int(OmegaConf.select(config, "econ.stub_dataset_size", default=1))
        print(f"[econ] No state_bank_path: using EconStubDataset (size={_stub_n})")
        train_dataset = EconStubDataset(_stub_n, _def_y)
        val_dataset = EconStubDataset(max(1, _stub_n), _def_y)

    return train_dataset, val_dataset, econ_collate
