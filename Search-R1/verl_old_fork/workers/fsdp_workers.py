...
from verl.utils.fsdp_utils import offload_fsdp_optimizer, offload_fsdp_param_and_grad, load_fsdp_optimizer, \
    load_fsdp_param_and_grad, materialize_adamw_optimizer_state
...
def _hf_attn_implementation() -> str:
    """Prefer FlashAttention2 when flash_attn is installed; else SDPA (no extra package)."""
    forced = os.environ.get('VERL_ATTN_IMPLEMENTATION', '').strip()
    if forced in ('sdpa', 'eager', 'flash_attention_2'):
        if forced == 'flash_attention_2':
...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        # ref 单独一套标志：actor_rollout_ref 混合角色下若用 elif ref 会永远读不到 ref 配置
        self._is_ref_offload_param = False
        self._is_ref_offload_grad = False
        if self._is_ref:
            self._is_ref_offload_param = self.config.ref.fsdp_config.get('param_offload', False)
            self._is_ref_offload_grad = self.config.ref.fsdp_config.get('grad_offload', False)
    ...
    def _build_model_optimizer(self,
        ...
        if self._is_actor:
            ...
            # optimizer offload：动量在 CPU 上预建，避免 init 时连续两次 GPU zeros_like 顶满显存；
            # 非 offload 保持 PyTorch 首次 step 再 lazy 分配（全在 GPU）。
            if self._is_offload_optimizer:
                materialize_adamw_optimizer_state(actor_optimizer, momentum_on_cpu=True)
    ...
    def init_model(self):
        ...
        if self._is_actor:
            ...
            # optimizer_offload 时若在 update_actor 开头就把 Adam 状态搬上 GPU，backward 峰值仍会爆显存；
            # 改为在 dp_actor._optimizer_step 里 step 前再 load、step 后 offload。
            if self._is_offload_optimizer:
                opt = self.actor_optimizer

                def _pre_opt():
                    load_fsdp_optimizer(optimizer=opt, device_id=torch.cuda.current_device())

                def _post_opt():
                    offload_fsdp_optimizer(optimizer=opt)

                self.actor._verl_optimizer_pre_step = _pre_opt
                self.actor._verl_optimizer_post_step = _post_opt
        ...
        if self._is_ref:
            ...
            if self._is_ref_offload_param:
                offload_fsdp_param_and_grad(module=self.ref_module_fsdp, offload_grad=self._is_ref_offload_grad)
    ...
    def update_actor(self, data: DataProto):
        ...
        # optimizer_offload：不在此处 load，见 init_model 里挂到 actor 上的 _verl_optimizer_pre_step
        ...
        with self.ulysses_sharding_manager:
            ...
            # 非 Ulysses：整条 batch 若一直在 GPU，backward 后仍占大量显存，load_fsdp_optimizer 再搬 Adam
            # 状态（~28GiB×2）时易 OOM。dp_actor 里会按 micro_batch .cuda()，此处把 batch 放到 CPU。
            if self.ulysses_device_mesh is None and data.batch is not None:
                _gpu_batch = data.batch
                data.batch = _gpu_batch.to('cpu', non_blocking=True)
                del _gpu_batch
                torch.cuda.empty_cache()
            # perform training
