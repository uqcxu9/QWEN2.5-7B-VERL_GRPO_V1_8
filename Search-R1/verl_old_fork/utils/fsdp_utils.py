...
def materialize_adamw_optimizer_state(optimizer: torch.optim.Optimizer, *, momentum_on_cpu: bool) -> None:
    """Pre-allocate AdamW lazy state so the first ``.step()`` does not run PyTorch lazy init.

    If ``momentum_on_cpu`` is True (used with optimizer state offload), allocate ``exp_avg`` /
    ``exp_avg_sq`` on **CPU** so init does not need two huge GPU allocations; ``load_fsdp_optimizer``
    moves them to GPU immediately before ``step``.

    If False, allocate on the parameter device (same as PyTorch ``zeros_like``), for setups that
    keep optimizer state on GPU and do not use deferred load.
    """
    ...
    try:
        ...
        get_scalar_dtype = adam_mod._get_scalar_dtype
        device_dtype_check = adam_mod._device_dtype_check_for_fused
    except (AttributeError, ImportError):
        get_scalar_dtype = lambda is_fused=False: torch.float32
        device_dtype_check = lambda p: None
    ...
    for group in optimizer.param_groups:
        ...
        for p in group['params']:
            ...
            if len(st) > 0:
                continue
            scalar_dtype = get_scalar_dtype(is_fused=fused)
            if fused:
                device_dtype_check(p)
            if capturable or fused:
                st['step'] = torch.zeros((), dtype=scalar_dtype, device=p.device)
            else:
                st['step'] = torch.tensor(0.0, dtype=scalar_dtype)
            if momentum_on_cpu:
                # CPU: torch.zeros 不接受与部分版本组合的 memory_format
                st['exp_avg'] = torch.zeros(p.shape, dtype=p.dtype, device='cpu')
                st['exp_avg_sq'] = torch.zeros(p.shape, dtype=p.dtype, device='cpu')
                if amsgrad:
                    st['max_exp_avg_sq'] = torch.zeros(p.shape, dtype=p.dtype, device='cpu')
            else:
