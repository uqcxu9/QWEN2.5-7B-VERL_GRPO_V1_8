...
class DataProto:
    ...
    def from_dict(cls, tensors: Dict[str, torch.Tensor], non_tensors=None, meta_info=None, num_batch_dims=1):
        ...
        assert len(tensors) > 0 or len(non_tensors) > 0, 'tensors and non_tensors cannot both be empty'

        if len(tensors) == 0:
            first_val = next(iter(non_tensors.values()))
            batch_size = (int(first_val.shape[0]),)
            tensor_dict = TensorDict(source={}, batch_size=torch.Size(batch_size))
        else:
            batch_size = None
            pivot_key = None
